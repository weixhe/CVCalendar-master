...
struct CVLunar {
    ...
    func formatToDate() -> Date {
        ...
        return formater.date(from: "\(year)-\(month)-\(day)")!
    }
}
...
struct CVLunarSolarConverter {
    
    /// 判断某年是否是闰年
    static func isLeapYear(y: Int) -> Bool {
        return ((y % 4 == 0 && y % 100 != 0) || (y % 400 == 0));
    }
    
    /// 农历转公历
    static func lunarToSolar(lunar: CVLunar) -> CVSolar {
        ...
        let solar11 = CVLunarSolarConverter.solar_1_1[lunar.year - CVLunarSolarConverter.solar_1_1[0]];
    ...
    /// 公历转农历
    static func solarToLunar(solar: CVSolar) -> CVLunar {
        var lunar = CVLunar();
        var index = solar.year - CVLunarSolarConverter.solar_1_1[0];
        let data = (solar.year << 9) | (solar.month << 5) | (solar.day);
        var solar11 = 0;
        ...
        let m = getBitInt(solar11, length: 4, shift: 5);
        let d = getBitInt(solar11, length: 5, shift: 0);
        var offset = solarToInt(solar.year, m: solar.month, d: solar.day) - solarToInt(y, m: m, d: d);
...
// MARK: - 私有方法
fileprivate extension CVLunarSolarConverter {
    
    static func getBitInt(_ data: Int, length: Int, shift: Int) -> Int {
        return (data & (((1 << length) - 1) << shift)) >> shift;
    }
    
    // WARNING: 日期1582年10月之前是不精确的
    static func solarToInt(_ y: Int, m: Int, d: Int) -> Int {
        let m = (m + 9) % 12;
        let y = y - m / 10;
        return 365 * y + y / 4 - y / 100 + y / 400 + (m * 306 + 5) / 10 + (d - 1);
    }
    
    static func solarFromInt(_ g: Int) -> CVSolar {
        ...
        let solar = CVSolar(year: y, month: mm, day: dd)
        return solar;
    }
